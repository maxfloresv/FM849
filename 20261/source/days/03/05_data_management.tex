...
\begin{document}
\classtitle{5}{Manejo de Datos en Python.}
...
\begin{frame}{Contenidos de hoy}
    ...
    \begin{itemize}%[<+->]
        ...
        \item Introducción al paquete Pandas.
...
\begin{frame}{Como trabajaremos los datos?}
...
\begin{itemize}
    ...
    \item El paquete \emph{Pandas} ofrece muchas funciones y estructuras de datos para procesar este tipo de datos.
\end{itemize}
...
\vspace{5mm}
    \begin{itemize}
        \item Contiene estructuras y herramientas para manejar datos tabulares de manera conveniente.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\linewidth]{days/03/image/dataframes.png}
...
Que devuelve el siguiente codigo ?
...
\begin{figure}
    \centering
    \input{tikz_figure/df_base2.tex}
    %\caption{Representación gráfica de un DataFrame de Pandas}
\end{figure}

Que devuelve el siguiente codigo ? (NOTAR QUE LOS INDICES CAMBIARON.)
...
\begin{frame}[fragile]{Acceder a columnas}

En Python, podemos acceder a los atributos de objetos usando un [ ] despues del objeto.

\begin{itemize}
    \item $\code{df[ col ] }$ devuelve una Serie generada con la columna $col$.
\end{itemize}
...
Como podríamos obtener las columnas Age y City simultaneamente ? HINT: Use listas de Python.
...
\begin{itemize}
    \item $\code{df.read\_csv()}$ para cargar un dataset desde un archivo csv o URL.
    \item $\code{df.head()}$ para mirar las primeras filas.
    \item $\code{df.tail()}$ para mirar las ultimas filas.
    \item $\code{df.shape}$ para obtener las dimensiones de la tabla.
    \item $\code{len(df)}$ para obtener el numero de filas en la tabla.
    ...
    \item $\code{df.iloc[ k ]}$ para obtener la fila ubicada en la posición $k$.
    \item $\code{df.loc[index]}$ para obtener la fila asignada al indice $index$.
    \item $\code{df.to\_csv()}$ para guardar el DataFrame en un archivo csv.
\end{itemize}
...
Lo último que vamos a ver es como guardar un objeto de tipo DataFrame, podemos guardar este usando las siguientes funciones (Tabla \ref{tab:save_data}).
