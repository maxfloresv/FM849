\documentclass[11pt,c,aspectratio=169,spanish]{beamer}
\usepackage{borelian}
...
\begin{frame}[fragile]
\frametitle{Medidas de tendencia central: Mediana}
...
\begin{frame}[fragile]
\frametitle{La interpretación estadística \textbf{SÍ} importa}
...
\begin{itemize}
    ...
    \item ¿Qué pasa si sólo nos enfocamos en informar sobre el promedio? Asumamos que el chileno promedio no tiene suficientes conocimientos estadísticos...
\end{itemize}
\end{frame}
...
Las \textbf{medidas de variabilidad} nos permiten describir cuan similares o diferentes son los datos de un conjunto. 

La \textbf{varianza} es el promedio de las diferencias cuadradas entre los valores de un conjunto y su media. Podemos pensar que estamos midiendo una \emph{distancia} (concepto que veremos mas adelante).

\[ \textbf{var}(X) = \frac{1}{n-1} \sum^n_{i=1} (x_i - \bar{X})^2 \]

\begin{lstlisting}[language=Python]
import numpy as np

a = [1,1,1,1]
print( np.var(a) ) # retornara  0

a = [1,2,3,4]
print( mean(a) )    # retornara  
\end{lstlisting}
    ...
    \begin{columns}
        \begin{column}{0.45\linewidth}
            ...
            \begin{block}{Definición}
                \justifying
                Las medidas de variabilidad se vinculan con estadísticos que permiten describir la dispersión de los datos en una muestra con respecto a una medida de tendencia central (usualmente la media).
            \end{block}
