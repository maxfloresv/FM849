...
\begin{document}
  ...
  \begin{frame}{Dificultad inherente a la regresión no lineal}
    ...
    \pause
    \textbf{¿Cómo se soluciona entonces este problema?} Acá entran los algoritmos de aproximación.
  \end{frame}

  \begin{frame}{Algoritmos de aproximación}
    \begin{block}{Definición}
      Los algoritmos de aproximación corresponden a una clase de algoritmos que buscan obtener soluciones aproximadas para problemas donde encontrar una solución exacta es muy difícil, dada la estructura matemática que tienen.
    \end{block}
    \pause
    En este curso, veremos uno de los algoritmos más importantes en la era de la inteligencia artificial moderna, conocido como el método del gradiente descendente.
  \end{frame}

  \begin{frame}{Método del gradiente descendente}
    \begin{columns}
      \column{0.45\linewidth}
      Pensemos que estamos en una colina y queremos llegar al punto más bajo. El problema es que estamos vendados, y sólo podemos sentir la inclinación del terreno en el punto donde estamos parados.
  ...
  \begin{frame}{Método del gradiente descendente}
    En términos prácticos, recordemos que lo que queremos hacer al resolver un problema con regresión no lineal (¡también!) es minimizar una función que me indica el error predictivo del modelo (función de costo).
    \pause
    \begin{itemize}
      \item Esta función de costo es como la ``colina'' del ejemplo anterior.
      \pause
      \item El punto más bajo de la colina corresponde al valor mínimo de la función de costo, es decir, el error predictivo más bajo.
    \end{itemize}
    \pause
    Hay casos donde la función puede tener múltiples mínimos, lo que dificulta la búsqueda de la solución óptima. En estos escenarios, se suele usar una versión alternativa, conocida como gradiente descendente estocástico, que introduce aleatoriedad en el proceso para evitar quedar atrapado en mínimos locales (¡el gradiente descendente normal usa todos los datos!).
  \end{frame}
\end{document}